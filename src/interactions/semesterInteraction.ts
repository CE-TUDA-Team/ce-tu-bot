...
export class SemesterInteraction extends InteractionSubHandler implements CommandInterface,ButtonInterface{
    ...
    async runCommand(interaction: CommandInteraction): Promise<void> {
        ...
        if (num <= 0) {
            await interaction.reply(`${gmember}: Bitte gib ein Bachelor-Semester ab 1 an.`);
            return;
        }
        if (num > 5) {
            num = 5;
        ...
        if(num === 1) {
            ...
            this.helper.memberHelper.memberAssignRole(interaction.member, rolename)
            await interaction.reply(`${gmember}: Willkommen auf Computational Engineering, du erhältst alle Erstsemestler Rollen.`);
            return;
        }
    }
